#!/usr/bin/env python3
"""
ResNet50 GradCAM Visualization Script for Brain Tumor Detection (Full Network Training)
This script loads the full network trained ResNet50 model and applies GradCAM visualization
to understand what regions the model focuses on for brain tumor MRI classification.
"""
...
class BrainTumorDataset:
    """Simple dataset class for loading brain tumor MRI images."""
...
def apply_gradcam_to_images(model, dataset, device, samples_per_class=5, save_dir="./gradcam_results/resnet50_brain_tumor_full"):
    ...
    try:
        ...
        for i, idx in enumerate(selected_indices):
            ...
            try:
                ...
                axes[0, 0].set_title(f'Original Brain MRI\nTrue: {class_name}', fontsize=11)
                axes[0, 0].axis('off')
                ...
                axes[1, 1].set_title('Brain Tumor Classification Probabilities', fontsize=11)
                ...
                fig.suptitle(f'Brain Tumor GradCAM Analysis - Full Network Training {correct_icon}\n'
...
def main():
    ...
    # Configuration
    config = {
        "model_path": "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/openMed/checkpoints/resnet50_brain_tumor_full/best_resnet50_brain_tumor_full_trained.pth",
        "data_path": "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/Brain_Cancer_Organized/test",
        "num_classes": 3,
        "num_images": 5,  # Number of images per class to analyze
        "save_dir": "./gradcam_results/resnet50_brain_tumor_full"
    }
    
    print("=== ResNet50 GradCAM Visualization for Brain Tumor Detection (Full Network Training) ===")
    ...
    try:
        ...
        print(f"❌ Error: {e}")
        print("Make sure you have trained the full network model first using resnet50_brain_tumor_full.py")
        return
    ...
    print(f"Expected: {config['num_images']} per class × {len(dataset.class_names)} classes = {config['num_images'] * len(dataset.class_names)} total images")
    ...
    print("\nGradCAM shows which parts of the brain MRIs the FULL NETWORK TRAINED model focuses on for brain tumor classification.")
    print("Red areas indicate high importance, blue areas indicate low importance.")
    print("Files are saved as: gradcam_[class_name]_[case_number]_full_training.png")
    print("\nComparison notes:")
    ...
    print("\nAnalysis includes:")
    print("- Original brain MRI image")
    print("- GradCAM heatmap showing model attention")
    print("- Overlaid visualization") 
    print("- Multi-class classification probabilities (Glioma, Meningioma, Tumor)")
