#!/usr/bin/env python3
"""
Vision Transformer (ViT) Fine-tuning Script for Chest X-ray Classification
This script provides fine-tuning of ViT pretrained on ImageNet,
where only the last classification head is trainable.
Based on the ResNet50 training framework but adapted for ViT architecture.
"""
...
class ViTFineTuned(nn.Module):
    ...
    def __init__(self, num_classes=2, freeze_features=True):
        ...
        print(f"ViT-B/16 loaded with ImageNet weights. Final layer: {num_features} -> {num_classes}")
        if freeze_features:
            print("All layers frozen except final classification head")
        else:
...
def train_vit_model(
        ...
        save_interval: int = 5,
        save_path: str = "./checkpoints/vit",
        use_mlflow: bool = True,
        experiment_name: str = "ViT_ChestXray_FineTuning",
        device=None,
        amp: bool = False,
        num_classes: int = 2,
        max_samples_per_class: int = None
):
    """Main training function for ViT fine-tuning."""
    ...
    if use_mlflow:
        ...
        # Log parameters
        mlflow.log_param("model_type", "ViT_B_16_FineTuned")
    ...
    # Create model
    model = ViTFineTuned(num_classes=num_classes, freeze_features=freeze_features)
    ...
    # Training setup
    info = f'''
        Model: ViT-B/16 Fine-tuned (ImageNet pretrained)
    ...
    for epoch in range(epochs):
        ...
        if test_accuracy > best_accuracy:
            ...
            if save_checkpoint:
                ...
                best_model_path = os.path.join(save_path, f"best_vit_finetuned.pth")
        ...
        if save_checkpoint and (epoch + 1) % save_interval == 0:
            ...
            checkpoint_path = os.path.join(save_path,
                                           ...
                                           f"_epoch{epoch+1}_vit_finetuned.pth")
...
def main():
    """Main function demonstrating ViT fine-tuning."""
    ...
    # Configuration for chest x-ray dataset with ViT fine-tuning
    config = {
        ...
        "experiment_name": "ViT_ChestXray_FineTuning_Full",
...
if __name__ == "__main__":
    ...
    mlflow_process = subprocess.Popen([
        'mlflow', 'ui', 
        '--host', '127.0.0.1', 
        '--port', '5001'  # Different port from ResNet50
