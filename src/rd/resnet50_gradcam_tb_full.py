#!/usr/bin/env python3
"""
ResNet50 GradCAM Visualization Script for TB Detection (Full Network Training)
This script loads the full network trained ResNet50 model and applies GradCAM visualization
to understand what regions the model focuses on for TB chest X-ray classification.
"""
...
class ResNet50FullTraining(nn.Module):
    ...
    def __init__(self, num_classes=2, freeze_features=False):
...
class TBDataset:
    """Simple dataset class for loading TB chest X-ray images."""
...
def load_model(model_path, device, num_classes=2):
    """Load the trained ResNet50 full training model."""
    model = ResNet50FullTraining(num_classes=num_classes, freeze_features=False)  # Full training model
    
    # Load the state dict
    if os.path.exists(model_path):
        checkpoint = torch.load(model_path, map_location=device, weights_only=False)
        model.load_state_dict(checkpoint)
        print(f"Full training model loaded from {model_path}")
    else:
...
def apply_gradcam_to_images(model, dataset, device, samples_per_class=5, save_dir="./gradcam_results/resnet50_tb_full"):
        ...
        for i, idx in enumerate(selected_indices):
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    # Get the probability for the other class
                    other_class = 1 - predicted_class
                    other_confidence = probabilities[0, other_class].item()
                
                print(f"  True: {class_name}")
                print(f"  Predicted: {dataset.class_names[predicted_class]} ({confidence:.3f})")
                print(f"  Other: {dataset.class_names[other_class]} ({other_confidence:.3f})")
                ...
                # Original image
                axes[0, 0].imshow(display_img_np)
                axes[0, 0].set_title(f'Original Chest X-ray\nTrue: {class_name}', fontsize=11)
                axes[0, 0].axis('off')
                
                # GradCAM heatmap
                axes[0, 1].imshow(colored_heatmap)
                axes[0, 1].set_title(f'GradCAM Heatmap\nTarget: {dataset.class_names[predicted_class]}', fontsize=11)
                axes[0, 1].axis('off')
                
                # Overlay
                axes[1, 0].imshow(overlay_img)
                axes[1, 0].set_title(f'Overlay Visualization\nConfidence: {confidence:.3f}', fontsize=11)
                axes[1, 0].axis('off')
                
                # Prediction probabilities
                class_probs = probabilities[0].cpu().numpy()
                colors = ['green' if i == predicted_class else 'lightcoral' for i in range(len(dataset.class_names))]
                bars = axes[1, 1].bar(range(len(dataset.class_names)), class_probs, color=colors)
                axes[1, 1].set_title('TB Classification Probabilities', fontsize=11)
                axes[1, 1].set_xlabel('Classes')
                axes[1, 1].set_ylabel('Probability')
                axes[1, 1].set_xticks(range(len(dataset.class_names)))
                axes[1, 1].set_xticklabels(dataset.class_names, rotation=45, ha='right')
                ...
                correct_icon = "✓" if predicted_class == true_label else "✗"
                prediction_text = "Correct" if predicted_class == true_label else "Incorrect"
                fig.suptitle(f'TB GradCAM Analysis - Full Network Training {correct_icon}\n'
                            f'{class_name} Case {sum(1 for j in range(i+1) if dataset.labels[selected_indices[j]] == true_label)} - {prediction_text} Prediction\n'
                            f'File: {os.path.basename(img_path)}\n'
                            f'Model: ResNet50 Full Training | Prediction: {dataset.class_names[predicted_class]} (Confidence: {confidence:.3f})', 
                            fontsize=12)
                ...
                # Save the figure
                case_number = sum(1 for j in range(i+1) if dataset.labels[selected_indices[j]] == true_label)
                save_path = os.path.join(save_dir, f'gradcam_{class_name.lower()}_{case_number}_full_training.png')
...
def main():
    ...
    # Configuration
    config = {
        "model_path": "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/openMed/checkpoints/resnet50_tb_full/best_resnet50_tb_full_trained.pth",
        "data_path": "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/TB_Chest_Radiography_Database_Organized/test",
        "num_classes": 2,
        "num_images": 5,  # Number of images per class to analyze
        "save_dir": "./gradcam_results/resnet50_tb_full"
    }
    
    print("=== ResNet50 GradCAM Visualization for TB Detection (Full Network Training) ===")
    print(f"Loading full training model from: {config['model_path']}")
    print(f"Loading test data from: {config['data_path']}")
    
    # Load the trained model
    try:
        model = load_model(config["model_path"], device, config["num_classes"])
        print("✓ Full network training model loaded successfully")
        
        # Count trainable parameters for comparison
        total_params = sum(p.numel() for p in model.parameters())
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"Model info: {total_params:,} total parameters, {trainable_params:,} were trainable during training")
        
    except FileNotFoundError as e:
        print(f"❌ Error: {e}")
        print("Make sure you have trained the full network model first using resnet50_tb_full.py")
        return
    ...
    print(f"Expected: {config['num_images']} Normal + {config['num_images']} Tuberculosis = {config['num_images'] * 2} total images")
    ...
    print("\nGradCAM shows which parts of the chest X-rays the FULL NETWORK TRAINED model focuses on for TB classification.")
    print("Red areas indicate high importance, blue areas indicate low importance.")
    print("Files are saved as: gradcam_normal_1_full_training.png, gradcam_tuberculosis_1_full_training.png, etc.")
    print("\nComparison notes:")
    print("- This model was trained with ALL LAYERS trainable (full network training)")
    print("- Compare with fine-tuned model results to see differences in attention patterns")
    print("- Full training may show different feature focus compared to transfer learning")
    print("\nAnalysis includes:")
    print("- Original chest X-ray image")
    print("- GradCAM heatmap showing model attention")
    print("- Overlaid visualization") 
    print("- Binary classification probabilities (Normal vs Tuberculosis)")
