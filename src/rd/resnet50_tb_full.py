#!/usr/bin/env python3
"""
ResNet50 Full Network Training Script for TB Chest X-ray Classification
...
Based on the TB fine-tuning framework but adapted for full network training.
Adapted for Tuberculosis vs Normal classification.
"""
...
class TBDataset(Dataset):
    """Dataset class for TB chest X-ray images."""
    ...
    def __init__(self, root_dir, transform=None, max_samples_per_class=None):
        ...
        # Get class directories (should be Normal and Tuberculosis)
...
def test_model(model, test_loader, epoch: int = -1, dataset: str = "", criterion=None, device=None):
    ...
    try:
        # For binary classification (TB vs Normal)
...
def train_resnet50_full_model(
    ...
    save_interval: int = 5,
    save_path: str = "./checkpoints/resnet50_tb_full",
    use_mlflow: bool = True,
    experiment_name: str = "ResNet50_TB_FullTraining",
    device=None,
    amp: bool = False,
    num_classes: int = 2,
    max_samples_per_class: int = None
):
    """Train ResNet50 model for TB classification with full network training."""
    ...
    print(f"Training ResNet50 for TB classification - FULL NETWORK TRAINING")
    ...
    for epoch in range(epochs):
        ...
        if test_accuracy > best_accuracy:
            ...
            if save_checkpoint:
                ...
                best_model_path = os.path.join(save_path, f"best_resnet50_tb_full_trained.pth")
        ...
        if save_checkpoint and (epoch + 1) % save_interval == 0:
            ...
            checkpoint_path = os.path.join(save_path,
                                           ...
                                           f"_epoch{epoch+1}_resnet50_tb_full_trained.pth")
    ...
    if use_mlflow:
        ...
        mlflow.sklearn.log_model(
            ...
            registered_model_name="resnet50-tb-full-model"
        )
        mlflow.end_run()
...
def main():
    """Main function demonstrating ResNet50 full network training for TB classification."""
    ...
    # Configuration for TB dataset with ResNet50 full network training
    organized_data_dir = "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/TB_Chest_Radiography_Database_Organized"
    
    # Check if organized data exists
    if not os.path.exists(organized_data_dir):
        print(f"Error: Organized TB dataset not found at {organized_data_dir}")
        print("Please make sure the TB dataset has been organized first.")
        return
    else:
        print(f"Using organized TB dataset at: {organized_data_dir}")
    
    config = {
        "root_path": organized_data_dir,
        ...
        "save_path": "./checkpoints/resnet50_tb_full",
        "use_mlflow": True,  # Use MLflow for experiment tracking
        "experiment_name": "ResNet50_TB_FullTraining",
        ...
        "num_classes": 2,  # Binary classification (Normal vs TB)
        "max_samples_per_class": None,  # Use full dataset
    }
    
    # Train model
    print("Starting ResNet50 FULL NETWORK training on TB chest X-ray dataset...")
    print("Using ImageNet pretrained weights, training ALL LAYERS")
    print("Classification: Normal vs Tuberculosis")
    ...
    print("Note: This was trained with FULL NETWORK TRAINING on the TB dataset.")
...
if __name__ == "__main__":
    ...
    mlflow_process = subprocess.Popen([
        'mlflow', 'ui', 
        '--host', '127.0.0.1', 
        '--port', '5003'  # Different port for full training experiment
    ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    print("MLflow UI started in background")
    print("Access the dashboard at: http://127.0.0.1:5003")
