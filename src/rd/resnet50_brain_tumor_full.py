#!/usr/bin/env python3
"""
ResNet50 Full Network Training Script for Brain Tumor MRI Classification
...
Based on the TB full training framework but adapted for brain tumor classification.
Adapted for 3-class Brain Tumor classification: Glioma, Meningioma, Tumor.
"""
...
class ResNet50FullTraining(nn.Module):
    ...
    def __init__(self, num_classes=3, freeze_features=False, enable_gradcam=False):
...
class BrainTumorDataset(Dataset):
    """Dataset class for brain tumor MRI images."""
    ...
    def __init__(self, root_dir, transform=None, max_samples_per_class=None):
        ...
        # Get class directories (should be brain_glioma, brain_menin, brain_tumor)
...
def test_model(model, test_loader, epoch: int = -1, dataset: str = "", criterion=None, device=None):
    ...
    try:
        ...
        # Sensitivity and Specificity for multi-class (weighted average)
        try:
            sensitivity = sensitivity_score(all_labels, all_predictions, average='weighted')
            specificity = specificity_score(all_labels, all_predictions, average='weighted')
        except:
...
def train_resnet50_full_model(
    ...
    save_interval: int = 5,
    save_path: str = "./checkpoints/resnet50_brain_tumor_full",
    use_mlflow: bool = True,
    experiment_name: str = "ResNet50_BrainTumor_FullTraining",
    device=None,
    amp: bool = False,
    num_classes: int = 3,
    max_samples_per_class: int = None
):
    """Train ResNet50 model for brain tumor classification with full network training."""
    ...
    print(f"Training ResNet50 for Brain Tumor classification - FULL NETWORK TRAINING")
    ...
    for epoch in range(epochs):
        ...
        if test_accuracy > best_accuracy:
            ...
            if save_checkpoint:
                ...
                best_model_path = os.path.join(save_path, f"best_resnet50_brain_tumor_full_trained.pth")
        ...
        if save_checkpoint and (epoch + 1) % save_interval == 0:
            ...
            checkpoint_path = os.path.join(save_path,
                                           ...
                                           f"_epoch{epoch+1}_resnet50_brain_tumor_full_trained.pth")
    ...
    if use_mlflow:
        ...
        mlflow.sklearn.log_model(
            ...
            registered_model_name="resnet50-brain-tumor-full-model"
        )
        mlflow.end_run()
...
def main():
    """Main function demonstrating ResNet50 full network training for brain tumor classification."""
    ...
    # Configuration for brain tumor dataset with ResNet50 full network training
    data_dir = "C:/Users/sriha/NEU/shlabs/HP_NVIDIA/Brain_Cancer_Organized"
    
    # Check if data exists
    if not os.path.exists(data_dir):
        print(f"Error: Brain tumor dataset not found at {data_dir}")
        print("Please make sure the brain tumor dataset exists.")
        return
    else:
        print(f"Using brain tumor dataset at: {data_dir}")
    ...
    config = {
        ...
        "save_path": "./checkpoints/resnet50_brain_tumor_full",
        "use_mlflow": True,  # Use MLflow for experiment tracking
        "experiment_name": "ResNet50_BrainTumor_FullTraining",
        ...
        "num_classes": 3,  # 3-class classification (Glioma, Meningioma, Tumor)
        "max_samples_per_class": None,  # Use full dataset
    }
    
    # Train model
    print("Starting ResNet50 FULL NETWORK training on brain tumor MRI dataset...")
    print("Using ImageNet pretrained weights, training ALL LAYERS")
    print("Classification: Glioma vs Meningioma vs Tumor")
    ...
    print("Note: This was trained with FULL NETWORK TRAINING on the brain tumor dataset.")
...
if __name__ == "__main__":
    ...
    mlflow_process = subprocess.Popen([
        'mlflow', 'ui', 
        '--host', '127.0.0.1', 
        '--port', '5005'  # Different port for brain tumor full training experiment
    ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    print("MLflow UI started in background")
    print("Access the dashboard at: http://127.0.0.1:5005")
